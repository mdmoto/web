# Maollar Web Platform (Originally Lilishop B2B2C)

**Note**: This project has been rebranded from Lilishop to Maollar and configured for AWS ECS backend deployment.

## AWS ECS Backend Configuration

All modules have been configured to connect to the AWS ECS backend:
- **Backend URL**: `http://52.33.226.250:8888`
- **WebSocket**: `ws://52.33.226.250:8888/lili/webSocket`

## Cloudflare Pages Deployment

This project is configured for deployment on Cloudflare Pages. Build configurations:

### Separate Applications (Recommended)

1. **Buyer App** (`maollar-buyer`)
   - Build command: `CF_MODULE=buyer npm run build`
   - Build output directory: `buyer/dist`
   
2. **Manager App** (`maollar-manager`)
   - Build command: `CF_MODULE=manager npm run build`
   - Build output directory: `manager/dist`
   
3. **Seller App** (`maollar-seller`)
...
- `npm run build:buyer` - Build buyer module only
- `npm run build:manager` - Build manager module only
- `npm run build:seller` - Build seller module only
- `npm run build:im` - Build IM module only

---

## Original Lilishop Documentation
